...
$prefDoctorSpec = $doctor ['doctor_cat'];
...
$queryDoctors = "SELECT person.email, person.fname, person.lname, doctor.doctor_cat, doctor.doctorid, ROUND(AVG(doctorreview.rating),2) FROM person INNER JOIN doctor on person.email=doctor.email inner join appointment on doctor.doctorid=appointment.doctorid inner join doctorreview
 								 on appointment.appid = doctorreview.appid WHERE person.role='doctor' GROUP BY appointment.doctorid";
$getDoctors = mysqli_query ( $conn, $queryDoctors );
$doctors = mysqli_fetch_all ( $getDoctors );

$queryHistory = "SELECT appid, doctor.doctor_cat, appointment.date, doctor.doctor_name, appointment.prescription,categories.name
...
$queryUpcApp = "SELECT appointment.appid, doctor.doctor_cat, person.fname, person.lname, appointment.date,
								TIME_FORMAT(appointment.time, '%H:%i'), appointment.doctorid FROM appointment
								inner join doctor on appointment.doctorid = doctor.doctorid inner JOIN person
								on doctor.email = person.email where patientid=$patientID and date > CURDATE()
		...
		<div class="header">
			<div class="container">
				...
				<div class="top-nav">
					...
					<ul class="nav1">
						<li><a href="home.php" class="active">Личный кабинет</a></li>
						<li><a href="makeAppointment.php">Записаться к врачу</a></li>
							...
							<div class="features-text">
								...
								<div class='tg-wrap'>
									...
									foreach ( $appHist as $app ) {
										echo "<tr><td>$app[2]</td><td> $app[4] $app[3]</td><td>$app[5]</td>";
										echo "<td><a href='viewAppointment.php?appId=" . $app [0] . "'>Посмотреть и оценить</a></td></tr>";
									}
									echo "
								</table>
								</div>";
							...
							<div class="features-text">
								...
								if (count ( $upcApp ) > 0) {
									...
									foreach ( $upcApp as $app ) {
										...
										echo "<td><a href='makeAppointment.php?appId=" . $app [0] . "'>Изменить</a><br />";
										echo "<a href='CancelApp.php?appId=" . $app [0] . "'>Отменить</a></td></tr>";
									}
									echo "</table></div>";
								} else {
									echo "<p>Нет ближайщих записей</p>";
								}
								?>
